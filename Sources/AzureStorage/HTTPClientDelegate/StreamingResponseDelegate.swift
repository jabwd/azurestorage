...
typealias onReceiveHeadCallback = (_ task: HTTPClient.Task<Void>, _ head: HTTPResponseHead) -> Void
typealias onReceiveBodyPartCallback = (_ task: HTTPClient.Task<Void>, _ buffer: ByteBuffer) -> Void
typealias onFinishRequest = (_ result: Result<HTTPClient.Task<Void>, Error>) -> Void

public final class StreamingResponseDelegate2: HTTPClientResponseDelegate {
  ...
  private let responsePromise: EventLoopPromise<HTTPClient.Response>
  private var writer: BodyStreamWriter?
  private var provisionalRespones: HTTPClient.Response
  private var completedPromise: Bool = false

  init(response: HTTPClient.Response, responsePromise: EventLoopPromise<HTTPClient.Response>) {
...
//    })
    ...
    buffer.writeBytes([])
    provisionalRespones.body = .init(buffer: ByteBuffer())
